Public Class StudentSubjectManagementForm
    ...
    Private Sub LoadEnrolledClass()
        ...
        cmd.CommandText = "SELECT co.idclass_offering as 'Class ID', CONCAT(s.subject_name, ' (',s.units,') Units ') as SD, 
    ...
    Private Sub LoadAvailableClass()
        ...
        cmd.CommandText = "SELECT co.idclass_offering, CONCAT(s.subject_name, ' (',s.units,') Units ', co.day, ' (Room ', co.room,')') as SD 
                           FROM class_offering as co JOIN subject as s JOIN teacher as t JOIN student_enroll_class as sec WHERE co.idsubject = s.idsubject AND co.idteacher = t.idteacher
                           AND sec.idclass_offering = co.idclass_offering AND sec.idstudent<>@id;"
